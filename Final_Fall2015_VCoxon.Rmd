...
```{r}
par(mfrow = c(1,1))
fit1 <- lm(sprinters$finish ~ sprint.model1$fitted.values)
...
#Option2
ggplot(sprinters,aes(x=year,y=finish,colour=as.factor(women))) + geom_point() + theme_bw() + stat_smooth(method='lm', formula = y~x) 
```
...
_Model2log differs in specification from Model1 because the outcome variable 'finish' has been log transformed to address this variable's obvious right skew. This transformation will change how we interpret the explanatory variable Beta coefficients to "a one unit increase in Xi is predicted to increase log(finish) by $exp(\beta_X_i)"$._
...
_Model3 assumes that the effect of 'year' is nonlinear. This must be incorporated into the original model specification (Model1) and the interaction term 'women:year'._
...
_Of the three models specified, I find Model3 to be the "best" model because the distribution of the residuls are relatively normally distributed, the residuals seem to be randomly distributed around zero, and the residual variance appears to be more constant across finish times in this model.  Additionally, the plot of standardized residuals is better in Model3  because the red line is flatter in Model 3 compared to the others and two of the potentially high leverage points(42,25) are closer to the mean0._ 

(@) (6 points) Do these models have different predictions for the Olympics of 2156? Why or why not?
```{r}
year_hyp <- seq(from = min(sprinters$year), to = 2156, by =4)
...
_Based on the histograms discussed in #12, I could have log transformed several variables to better address deal the right skew in the data. I chose to create a model (model4) that log transformed 'oilL5' and 'GDPpcL5' while also creating a dummy variable for the values of 'islam' above the median of 0.0070. I believe there is a connection between oil production and GDP; my logic was that if I am log transforming one, I need to log transform the other. I chose to use the median values of 'islam' as a cut point to correct for the effects of high value observations on the 'islam' mean._ 

(@) (4 points) For your new model, record and compare the adjusted $R^2$, AIC score, and BIC score for your new model.
```{r}
library(stargazer); library(knitr)
stargazer(model1, model4, model5, type = 'text', column.labels = c('Model1', 'Model 4','Model 5'), model.names = FALSE, model.numbers = FALSE)

AIC(model1,model4,model5)
BIC(model1,model4,model5)
```
...
_We can be 95 % confident that the __average__ expected change predicted by Model 4 in 'regime1'  resulting from a given change in 'oilL5' from the 50th percentile to the 95th percentile holding all other variables constant is a decrease of approximately 0.84 units._
...
library(stats4)
LL <- function(beta0, beta1, beta2, beta3, beta4, beta5, beta6, mu, sigma) {
    R = rossoil$regime1 - rossoil$oecd * beta6 - (rossoil$islam * rossoil$islam_dummy) * beta5 - rossoil$islam_dummy * beta4 - log(rossoil$GDPpcL5) * beta3  - rossoil$metalL5  * beta2 - rossoil$oil_mod * beta1 - beta0
    #
...
m.l.e. = mle(LL, start = list(beta0 = 1, beta1 = 1, beta2 = 1, beta3 = 1, beta4 = 1, beta5 = 1, beta6 = 1, sigma = 1), fixed = list(mu = 0), nobs = length(rossoil$regime1))

summary(m.l.e.)

LL2 <- function(beta0, beta1, beta2, beta3, beta4, beta5, beta6, mu, sigma) {
    R = rossoil$regime1 - rossoil$oecd * beta6 - (rossoil$islam * rossoil$islam_dummy) * beta5 - rossoil$islam_dummy * beta4 - log(rossoil$GDPpcL5) * beta3  - rossoil$metalL5  * beta2 - rossoil$oilL5 * beta1 - beta0 
    #
    R = suppressWarnings(dnorm(R, mu, sigma, log = TRUE))
    #
    -sum(R)
...
m.l.e.2 = mle(LL2, start = list(beta0 = 1, beta1 = 1, beta2 = 1, beta3 = 1, beta4 = 1, beta5 = 1, beta6 = 1, sigma=1),
             fixed = list(mu = 0),
             nobs = length(rossoil$regime1))

summary(m.l.e.2)
```

_Iam going to respecify a model based on the significance levels of explanatory variables in Models 4 and 5._

```{r}
LL3 <- function(beta0, beta1, beta2, beta3, mu, sigma) {
    R = rossoil$regime1 - (rossoil$islam * rossoil$islam_dummy) * beta3 - rossoil$islam_dummy * beta2 - log(rossoil$GDPpcL5) * beta1 - beta0
    #
    R = suppressWarnings(dnorm(R, mu, sigma, log = TRUE))
    #
    -sum(R)
...
m.l.e.3 = mle(LL3, start = list(beta0 = 1, beta1 = 1, beta2 = 1, beta3 = 1, sigma = 1), fixed = list(mu = 0), nobs = length(rossoil$regime1))

summary(m.l.e.3)

ols.estimate = lm(regime1 ~ log(GDPpcL5) + factor(islam_dummy) + islam:islam_dummy, data = rossoil)
...
L4 <- prod(dnorm(ols.estimate$residuals, mean = 0, sd = summary(ols.estimate)$sigma))
c(L1, L2, L3, L4)
```
...
```{r}
stargazer(model1,model4,model5,ols.estimate, type = 'text', column.labels = c('Model1', 'Model 4','Model 5', 'OLS Estimate'), model.names = FALSE, model.numbers = FALSE)

AIC(model1,model4,model5,ols.estimate)
BIC(model1,model4,model5,ols.estimate)
```

_After conducting a comparison of the models and their corresponding AIC and BIC scores it appears that the OLS Estimate Model balances parsimony and description with an R-square value that acccounts for 56.3%  of variation in the outcome 'regime1'._
_I do have a  preference for the Model 5 specification mainly because it has a higher R-square value meaning this modelaccounts for approximately 61.5% of the variation in the outcome 'regime1'. Model 5 may need some non-linear transformation tweaking to become more descriptive, but I believe it may be pointing in the right direction._
_Of the models tested, the OLS Estimate Model, has the lowest BIC score at 340.7228, with 5 df. It also has the most significant F statistic at 28.312*** (df = 3;66); **of the models** evaluated, it appears that the best model is the OLS Estimate._
...
_I have continuing concerns regarding the best model specification to use with this data, particularly because the models do not do a better job  accounting for the variation in the outcome (it only seems to account for a little more than half; this seems no better than chance). I think we might be missing some important explanatory variables that could be included in the model like women's rights which could be a proxy measure of oppression, or possibly a corruption index._ 
...
#NOTES:
**I would like to know how to log transform 'islam' in such a way that it gets "turned on" in the equation when the 'islam dummy' = 1, but log(islam) is "turned off" when the 'islam_dummy' = 0.**